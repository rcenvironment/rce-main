...
public class ToolConfigurationPage extends ToolIntegrationWizardPage {
    ...
    private void updatePageComplete() {
        ...
        final List<Map<String, String>> launchSettings =
            (List<Map<String, String>>) configurationMap.get(IntegrationConstants.KEY_LAUNCH_SETTINGS);
    ...
    private void updateTable() {
        ...
        if (configurationMap.get(IntegrationConstants.KEY_LAUNCH_SETTINGS) != null) {
            ...
            for (Map<String, String> currentConfig : configs) {
                ...
                item.setText(0, currentConfig.get(IntegrationConstants.KEY_HOST));
                item.setText(1, currentConfig.get(ToolIntegrationConstants.KEY_TOOL_DIRECTORY));
                item.setText(2, currentConfig.get(IntegrationConstants.KEY_VERSION));
    ...
    private void addSelection() {
        ...
        if (configurationMap.get(IntegrationConstants.KEY_LAUNCH_SETTINGS) != null) {
            configs = (List<Map<String, String>>) configurationMap.get(IntegrationConstants.KEY_LAUNCH_SETTINGS);
        } else {
            configurationMap.put(IntegrationConstants.KEY_LAUNCH_SETTINGS, configs);
        }
    ...
    private void editSelection(TableItem[] selection) {
        ...
        if (selection != null && selection.length > 0) {
            ...
            for (Map<String, String> currentConfig : configs) {
                if (currentConfig.get(IntegrationConstants.KEY_HOST).equals(selection[0].getText(0))
            ...
            if (exit == 0) {
                ...
                configs.add(wtcd.getConfig());
                configurationMap.remove(IntegrationConstants.KEY_LAUNCH_SETTINGS);
                configurationMap.put(IntegrationConstants.KEY_LAUNCH_SETTINGS, configs);
                updateTable();
            }
        }
