 ...
 * 
 * @author Sascha Zur
 * @author Kathrin Schaffert (refactoring in validateInput(): ComponentDescriptionValidator)
 */
...
public class WizardToolConfigurationDialog extends Dialog {
     ...
     * @param title
     * @param context current {@link IntegrationContext}
     * @param configuration the containing endpoint manager
     */
    public WizardToolConfigurationDialog(Shell parentShell, String title, IntegrationContext context) {
        super(parentShell);
        config = new HashMap<>();
        config.put(IntegrationConstants.KEY_LIMIT_INSTANCES, "true");
        config.put(IntegrationConstants.KEY_LIMIT_INSTANCES_COUNT, "10");
        this.title = title;
    ...
    public WizardToolConfigurationDialog(Shell parentShell, String title, Map<String, String> config, IntegrationContext context) {
        super(parentShell);
        this.config = config;
        ...
        this.title = title;
        this.context = context;
        if (config.get(IntegrationConstants.KEY_LIMIT_INSTANCES) == null
            && config.get(ToolIntegrationConstants.KEY_LIMIT_INSTANCES_OLD) == null) {
            config.put(IntegrationConstants.KEY_LIMIT_INSTANCES, "false");
            config.put(IntegrationConstants.KEY_LIMIT_INSTANCES_COUNT, "10");
        }
        setShellStyle(SWT.RESIZE | SWT.MAX | SWT.APPLICATION_MODAL);
    }
    ...
    private void updateInitValues() {
        if (config.get(IntegrationConstants.KEY_HOST) != null) {
            config.get(IntegrationConstants.KEY_HOST);
        }
        ...
        if (config.get(IntegrationConstants.KEY_VERSION) != null) {
            versionText.setText(config.get(IntegrationConstants.KEY_VERSION));
        }
        ...
        if (config.get(IntegrationConstants.KEY_LIMIT_INSTANCES) != null) {
            setInstanceLimit(IntegrationConstants.KEY_LIMIT_INSTANCES);
    ...
    private void setInstanceLimit(String key) {
        if (Boolean.parseBoolean(config.get(key))) {
            ...
            limitInstancesText.setText(config.get(IntegrationConstants.KEY_LIMIT_INSTANCES_COUNT));
        } else {
            limitInstancesButton.setSelection(false);
    ...
    private void saveAllConfig() {
        ...
        config.put(IntegrationConstants.KEY_HOST, IntegrationConstants.VALUE_LOCALHOST);
        /*
        ...
        config.put(IntegrationConstants.KEY_VERSION, versionText.getText());

        config.put(IntegrationConstants.KEY_LIMIT_INSTANCES, "" + limitInstancesButton.getSelection());
        if (limitInstancesButton.getSelection()) {
            config.put(IntegrationConstants.KEY_LIMIT_INSTANCES_COUNT, limitInstancesText.getText());
        }
    ...
    protected void validateInput() {
        ...
        boolean isValidLimitation = true;
        setMessage("");
        ...
        Optional<String> limitationMessage = validator.validateParallelExecution(limitInstancesText, limitInstancesButton);
        if (limitationMessage.isPresent()) {
            setMessage(limitationMessage.get());
            isValidLimitation = false;
        }
