...
/**
 * Dialog for removing an integrated component configuration.
 * 
 * @author Sascha Zur
 * @author Adrian Stock
...
public class DeactivateIntegrationDialog extends Dialog {
    ...
    private String[] selectedIntegrations = null;
    ...
    public DeactivateIntegrationDialog(Shell parent, Set<String> integratedConfigurations, Collection<IntegrationContext> contexts) {
        super(parent);
        ...
        for (String config : integratedConfigs) {
            for (IntegrationContext c : contexts) {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        new Label(container, SWT.NONE).setText("Choose configuration to deactivate: ");
        integratedComponentList = new List(container, SWT.MULTI | SWT.BORDER | SWT.V_SCROLL);
        ...
        integratedComponentList.setLayoutData(toolListData);
        for (String key : integrationMapping.keySet()) {
            integratedComponentList.add(key);
        }
        integratedComponentList.addSelectionListener(new SelectionListener() {

            @Override
            public void widgetSelected(SelectionEvent arg0) {
                if (integratedComponentList.getSelectionCount() > 0) {
                    selectedIntegrations = integratedComponentList.getSelection();
                }
    ...
    public String[] getSelectedTools() {
        if (selectedIntegrations != null && selectedIntegrations.length > 0) {
            String[] selectedIDs = new String[selectedIntegrations.length];
            for (int i = 0; i < selectedIntegrations.length; i++) {
                selectedIDs[i] = integrationMapping.get(selectedIntegrations[i]);
            }
            return selectedIDs;
        }
        return new String[0];
    ...
    public void setSelectedTools(String[] selectedTools) {
        this.selectedIntegrations = selectedTools;
    }

    private void readExistingConfigurations(Collection<IntegrationContext> contexts) {
                        ...
                        for (File f : files) {
                            if (f.getName().equals(context.getConfigurationFilename())) {
                                try {
                                    ...
                                    allConfigurations.put((String) configurationMap.get(IntegrationConstants.KEY_COMPONENT_NAME),
                                        configurationMap);
                                } catch (IOException e) {
