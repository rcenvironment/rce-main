 ...
 * @author Sascha Zur
 */
public class ShowDeactivateIntegrationHandler extends AbstractHandler {

    private static final Log LOGGER = LogFactory.getLog(ShowDeactivateIntegrationHandler.class);
    ...
    public Object execute(final ExecutionEvent event) throws ExecutionException {
        ...
        DeactivateIntegrationDialog dialog =
            new DeactivateIntegrationDialog(null, integrationService.getActiveComponentIds(), registry.getAllIntegrationContexts());
        ...
        if (dialog.open() == 0) {
            ...
            for (String selectedTool : dialog.getSelectedTools()) {
                ...
                if (configuration != null) {
                    Object integrationTypeProperty = configuration.get(IntegrationConstants.INTEGRATION_TYPE);
    ...
    private void removeOrDeactive(ToolIntegrationService integrationService, DeactivateIntegrationDialog dialog,
        IntegrationContext context, File remove) {
        if (!dialog.getKeepOnDisk()) {
            ...
            try {
                ...
                configurationMap.put(IntegrationConstants.IS_ACTIVE, false);
